...
import type { NextAction, SessionMode, Specification } from "@/domain/types";
...
export function FinalReview({ specification, revision, mode, finalized, brainModel, realtimeModel, onNextActionsChange, onFinalize, onResume, onExit }: { specification: Specification; revision: number; mode: SessionMode; finalized: boolean; brainModel?: string | null; realtimeModel?: string | null; onNextActionsChange: (actions: NextAction[]) => void; onFinalize: () => void; onResume: () => void; onExit: () => void }) {
  const [fallback, setFallback] = useState<string | null>(null);
  const [notice, setNotice] = useState("");
  const [confirmingExit, setConfirmingExit] = useState(false);
  const markdown = useMemo(() => specificationToMarkdown(specification, { mode, finalized, brainModel, realtimeModel }), [specification, mode, finalized, brainModel, realtimeModel]);
